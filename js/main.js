...
function ensureLightbox() {
  ...
  lightbox.addEventListener('click', (event) => {
    if (event.target === lightbox || event.target.closest('.lightbox-close')) {
      ...
      shareLightboxImage();
    } else if (event.target.classList.contains('lightbox-img')) {
      toggleLightboxZoom();
    }
  });
...
function toggleLightboxZoom() {
  ...
  const btn = lightbox.querySelector('.lightbox-zoom i');
  const zoomed = wrap.classList.toggle('zoomed');
  if (btn) btn.className = zoomed ? 'fa-solid fa-magnifying-glass-minus' : 'fa-solid fa-magnifying-glass-plus';
}
...
function showLightboxAt(index) {
  ...
  if (zoomBtn) zoomBtn.className = 'fa-solid fa-magnifying-glass-plus';
